...
class RobotController:
    def __init__(self, websocket_client, webrtc_manager):
        ...
        self.terminal_data = ""
        self.terminal_data_sub = rospy.Subscriber('/terminal_data', String, self.terminal_data_callback)
    ...
    def terminal_data_callback(self, data):
        self.terminal_data = data.data
        # rospy.loginfo(rospy.get_caller_id() + "I heard %s", data.data)

    async def listener(self):
        while not rospy.is_shutdown():
            await self.webrtc_manager.send_data(self.terminal_data)
            await asyncio.sleep(1)
